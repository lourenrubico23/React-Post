...
const FeatureTable = ({isLoading, isFetching, feature, setItemEdit}) => {
  ...
  return (
    ...
    <div className="table-wrapper relative">
        {isFetching && <SpinnerFetching/>}
                                ...
                                {feature?.data.length === 0 && (
                                    ...
                                    {feature?.data.map((item, key) => (
                                        <tr className='hover:bg-accent/10' key={key}>
                                            ...
                                            <td>{item.feature_author}</td>
                                            <td>{item.feature_article}</td>
                                            <td>{item.feature_publish_date}</td>
                    ...
                    {store.isActive && <ModalConfirm position="center" queryKey="feature" endpoint={`/v1/feature/active/${id}`} isArchiving={isArchiving} />}

                    {store.isDelete && <ModalDelete position="center" queryKey="feature" endpoint={`/v1/feature/${id}`} />}
