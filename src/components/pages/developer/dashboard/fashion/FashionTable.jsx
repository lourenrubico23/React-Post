...
const FashionTable = ({isLoading, isFetching, fashion, setItemEdit}) => {
  ...
  return (
    ...
    <div className="table-wrapper relative">
        {isFetching && <SpinnerFetching/>}
                        <table>
                            ...
                            {isLoading && ( 
                                ...
                                {fashion?.data.length === 0 && (
                                    ...
                                    {fashion?.data.map((item, key) => (
                                        <tr className='hover:bg-accent/10' key={key}>
                                            <td>{counter++}</td>
                                            <td>{item.fashion_title}</td>
                                            <td>{item.fashion_category}</td>
                                            <td>{item.fashion_image}</td>
                                            <td>{item.fashion_author}</td>
                                            <td>{item.fashion_article}</td>
                                            <td>{item.fashion_publish_date}</td>
                                            <td className='table-action'>
                                            <ul>
                                                {item.fashion_is_active ? (
                                                    <>
                    ...
                    {store.isActive && <ModalConfirm position="center" queryKey="fashion" endpoint={`/v1/fashion/active/${id}`} isArchiving={isArchiving} />}

                    {store.isDelete && <ModalDelete position="center" queryKey="fashion" endpoint={`/v1/fashion/${id}`} />}
